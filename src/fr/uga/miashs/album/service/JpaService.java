...
public abstract class JpaService<K,V> implements GenericService<K,V>, Serializable {
	...
	@PostConstruct
	public void init () {
		em = emf.createEntityManager();//on fais des requete sur la bdd via cet entitymanager
	}
	...
	public void create(V v) throws ServiceException {
		...
		t.begin();
		em.persist(v);	//enregistre l'instance passé en param dans la bdd
		t.commit();
	}
	
	public V read(K id) {
		return em.find(cls, id); //cls = class de l'objet à récupéré
	}
